$(document).ready(function() {
    ...
    $("#SelectPro").bind("click", function() {
        ...
        // 打开
        art.dialog.open(bindDefaultInfo.postProUrl+"?w=" + $.trim(warehouse.val()) + "&pid=" + $.trim($("#InWarehouseId").html()+"&type="+$('#inType').val()), {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.code == "0") {
                        ...
                        if (data.code=='2') {
                            art.dialog.confirm('入库申请单需支付后等待平台审核，是否前往入库单列表查看？',function(){
                                window.location.href=bindDefaultInfo.warelist
                            },function(){
                                location.reload();
                            }); // 提示语句
                        }else{
