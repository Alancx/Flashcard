...
function changeCld() {
   var y,m;
   y = form1.SY.selectedIndex + 1900;
   m = form1.SM.selectedIndex;
   drawCld(y,m);
}
//用自定义变量保存当前系统中的年月日
var Today = new Date();
...
function initial(type) {
	var d = document.getElementById("txt_shengri").value;
	if (d !="") {
		if (IsDate(d)) {
			tY = parseInt(d.split('-')[0]);
			tM = parseInt(d.split('-')[1]) - 1;
		}
	}
	form1.SY.selectedIndex = tY - 1900;
	form1.SM.selectedIndex = tM;
	drawCld(tY, tM);
}