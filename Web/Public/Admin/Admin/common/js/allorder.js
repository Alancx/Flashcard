		...
		success: function(data) {
			if (data.code == "0") {
				...
				$(data.dataOrder).each(function(index, vo) {
					...
					else if(vo.PayName=="YE")
					{
						_payname="余额支付";
					}
					else if(vo.PayName=="JL")
					{
						_payname="奖励余额支付";
					}
					var cfstr='';
					// if (vo.Status==1) {
					// 	cfstr='<a data-toggle="modal" data-target="#edit_order" data-order="'+vo.OrderId+'" class="price_edit" >修改价格</a>&emsp;<a data-toggle="modal" data-target="#cfreight" data-order="'+vo.OrderId+'" class="cfreight" >修改运费</a>&emsp;';
					// };
					_html = '<tr><td colspan="7"></td></tr><tr><td colspan="7"><input type="checkbox" name="poid" id="" value="'+vo.OrderId+'" />  <span style="color:#000">订单号：' + vo.OrderId + '</span><span class="c-gray">' + _payname + '</span> &emsp; 商家：'+vo.storename+'</td></tr>';
					// <span class="pull-right">'+cfstr+'<a data-toggle="modal" data-order="' + vo.OrderId + '" class="data-order-info" data-target="#orderinfo">查看详情</a>&nbsp;-&nbsp;<a data-toggle="modal" data-order="' + vo.OrderId + '" class="data_order_message" data-target="#order_message">备注</a></span>
					$(vo.datason).each(function(sonindex, son) {
						...
						switch (_tempState) {
							...
							case 2:
							// if (vo.IsCheck==1) {
								_tempHtmlState = '<p>已付款</p>';
// <p id="s'+vo.OrderId+'"><button class="btn btn-default btn-outline status-button" type="button" onclick="prints(\''+vo.OrderId+'\');">打印发货单</button> &nbsp;<button class="btn btn-default btn-outline status-button" type="button" onclick="printlog(\''+vo.OrderId+'\');">打印快递单</button> &nbsp;<a href="javascript:;" data-toggle="modal" data-order="'+vo.OrderId+'" class="btn btn-outline btn-default status-button order_send" data-target="#ModelSend">发&nbsp;&nbsp;货</a></p>							// }else{
							// 	_tempHtmlState = '<p>已付款，等待发货</p><p id="s'+vo.OrderId+'"><button class="btn btn-default btn-outline status-button" type="button" onclick="prints(\''+vo.OrderId+'\');">打印发货单</button> <button class="btn btn-default btn-outline status-button" type="button" onclick="printlog(\''+vo.OrderId+'\');">打印快递单</button> &nbsp;<button class="btn btn-default btn-outline status-button" type="button" onclick="checks(\''+vo.OrderId+'\');">验 货</button></p>';
							// }
								break;
							case 3:
								_tempHtmlState = "<p>-</p>";
								break;
							case 4:
								_tempHtmlState = "<p>已完成</p>";
								break;
							case 5:
							if (vo.BackSuccess=='1') {
								_tempHtmlState = '<p>买家已退款</p><p><a href="javascript:;" class="btn btn-outline btn-default status-button">退款已提交(退款成功时间视银行处理时间而定)</a></p>';
							}else{
						...
						_html += '<tr><td class="image-cell"><img style="width:60px;height:60px;" src="' + binddefaultinfo.postRootUrl + son.ProLogoImg + '" alt="" /></td><td class="product-cell"><p><a href="' + binddefaultinfo.postWebRootUrl + "?pid=" + son.ProId + '" target="_blank" title="' + son.ProName + '">' + son.ProName + "</a></p><p>" + (son.Spec==null?"":son.Spec) + "</p></td><td><p class='"+vo.OrderId+'_'+son.ProIdCard+"_price'>" + Number(son.Price).toFixed(2) + "</p><p class='"+vo.OrderId+'_'+son.ProIdCard+"_count'>( " + son.Count + " 件)</p></td>";
						if (sonindex == 0) {
							_html += "<td " + rowspan + "><p>" + rname + "</p><p>" + vo.Tel + "</p></td><td " + rowspan + ">" + vo.Date + "</td><td " + rowspan + ">" + _tempHtmlState + "</td><td " + rowspan + "><p>" + Number(vo.Price).toFixed(2) + "</p><p><small class='c-gray'>(含运费：<span class='cf_"+vo.OrderId+"'>" + Number(vo.Freight).toFixed(2) + "</span>)</small></p></td>";
						}
						_html += "</tr>"
					});
					tbody.append(_html)
	...
	$(document).on("click", ".data-order-info", function() {
		...
		$.ajax({
			...
			success: function(datas) {
				...
				var _payname="-";
				if(data.PayName=="ALIPAY"){_payname="支付宝";}
				else if(data.PayName=="T")
				{
					_payname="微信支付";
				}
...
$(document).on('click','.getloginfo',function(){
	...
	$.ajax({
		...
		success:function(msg){
			var html="<h5 style='text-align:center'>"+ msg.stname +":"+ msg.stid +" <br><br>当前状态："+ msg.st +"</h5>";
			if (msg.status=='success') {
				if (msg.data) {
					$.each(msg.data,function(index,item){
						html+="<div class='loli'>"+item.context+" <div class='lotime'>"+item.time+"</div></div>";
					})
				};
			}else{
				html+="<div>查询失败</div>";
			}
			$('#exp_content').html(html);
		}
	})
})

//运费修改
$(document).on('click','.cfreight',function(){
	var oid=$(this).attr('data-order');
	$('#savefreight').attr('data-order',oid);
	$('#Freight').val('');
	$('#cfnotice').html(''); 
})
$(document).on('click','#savefreight',function(){
	var _this=$(this);
	...
	$.ajax({
		url:cfreighturl,
		type:"post",
		data:"oid="+oid+"&Price="+$('#Freight').val(),
		dataType:"json",
		success:function(msg){
			if (msg.status=='success') {
				$('#cl_close').click();
				$('#cfnotice').html('');
				$('.all_'+oid).html(msg.info.Price);
				$('.cf_'+oid).html(msg.info.Freight);
...
$(document).on('click','.price_edit',function(){
	var oid=$(this).attr('data-order');
	//拿订单信息
	$('#saveeditinfo').attr('data-order',oid);
	$('#ep_notice').html('').html('加载中...');
	$.ajax({
		url:getoinfochange,
		type:"post",
		data:"oid="+oid,
		...
		success:function(msg){
			if (msg.status=='success') {
				...
				$.each(data,function(index,item){
					_html+='<tr><td colspan="3"><div class="input-group"><span class="input-group-addon edit_order_proname" title="'+item.pname+'">'+item.pname+'</span><input type="number" value="'+item.Count+'" name="count[]" class="form-control" style="width:100px;" id=""><input type="text" name="price[]" id="" value="'+item.Price+'" class="form-control" style="width:100px;"><input type="hidden" name="pids[]" value="'+item.pid+'" /><input type="hidden" name="olids[]" value="'+item.ID+'" /></div></td></tr>';
				})
				$('#ep_tobdy').html(_html);
			}else{
				$('#ep_notice').html('获取订单信息失败');
			}
		}
	})
})
...
//保存修改信息
$(document).on('click','#saveeditinfo',function(){
	var oid=$(this).attr('data-order');
	$('#ep_notice').html('处理中...');
	$.ajax({
		url:saveditinfourl,
		type:"post",
		data:$('#oinfo').serialize()+"&oid="+oid,
		dataType:"json",
		success:function(msg){
			if (msg.status=='success') {
				var data=msg.data;
				//处理变更
				$.each(data,function(index,item){
					$('.'+oid+'_'+item.pid+'_price').html(item.price);
					$('.'+oid+'_'+item.pid+'_count').html("("+item.count +' 件)');
				})
				$('.all_'+oid).html(data.money);
				$('#ep_close').click();
				art.dialog.tips('修改成功');
			}else{
				$('#ep_notice').html('修改失败');
