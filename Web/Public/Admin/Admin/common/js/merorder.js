...
var getDatapager = function(type) {
	var data;
	var stoken=$('#merchant').val();
	if (stoken=='-1') {
		art.dialog.tips('请选择商户');
		return false;
	};
	if (type == "all") {
		data = {
		...
		success: function(data) {
			if (data.code == "0") {
				...
				$(data.dataOrder).each(function(index, vo) {
					...
					_html = '<tr><td colspan="7"></td></tr><tr><td colspan="7"><input type="checkbox" name="poid" id="" value="'+vo.OrderId+'" />  <span style="color:#000">订单号：' + vo.OrderId + '</span><span class="c-gray">' + _payname + '</span>&emsp;<span>商户：'+vo.storenames+'</span></td></tr>';
					// <span class="pull-right"><a data-toggle="modal" data-order="' + vo.OrderId + '" class="data-order-info" data-target="#orderinfo">查看详情</a>&nbsp;-&nbsp;<a data-toggle="modal" data-order="' + vo.OrderId + '" class="data_order_message" data-target="#order_message">备注</a></span>
	...
	$(document).on("click",".btn-js-export",function(){
		...
		if(ty=="default"){
			...
			if (stoken=='-1') {
				...
				location.href=binddefaultinfo.postExport+"?ty="+$(this).attr("data-export-type")+"&state="+input_state.val()+"&user_name="+$.trim($("#user_name").val())+"&start_time="+$.trim($("#start_time").val())+"&end_time="+$.trim($("#end_time").val())+"&buy_way="+ $.trim($("#buy_way").val())+"&stoken="+stoken+"&r="+(Math.random() * Math.random());
			}
		};
		if (ty=='newxls') {
