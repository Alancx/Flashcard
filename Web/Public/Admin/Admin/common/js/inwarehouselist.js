$(document).ready(function() {
    ...
    $(document).on("click",".par-out-a",function(){
        var paying=$(this).attr('data-paying');
        var cid=this.getAttribute('data-cid');
        var str='该操作将自动从主仓库中调拨出库对应商品';
        if (paying=='no') {
            str='该单据未支付！！！确定要同意入库申请吗？？？';
        };
        art.dialog.confirm(str,function(){
            art.dialog.tips('同意了');
            window.location.href=bindinfo.passurl+"?Ivid="+cid;
        },function(){
                            ...
                            _html = '<div class="timeline-item"><div class="row"><div class="col-xs-2 date"><i class="fa fa-file-text"></i><small>' + val.Date + '</small><br><small class="' + (val.Status == "已入库" ? "text-navy" : "text-danger") + '">' + val.Status + '</small></div><div class="col-xs-10 content"><p class="m-b-xs" style="border-width:1px;padding-bottom:10px;"><span class="label label-primary">系统单号：' + val.InWarehouseId + '，手工单号：' + val.InWarehouseNumber + '</span>&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-info">类型：' + _intype + '</span>&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-info">采购人：' + val.InputName + '</span>&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-warning">总数量：' + val.Count + '</span>&nbsp;&nbsp;&nbsp;&nbsp;<span class="label label-warning">总金额：' + Number(val.Money).toFixed(2)  + '</span><small class="pull-right text-navy">';
                                ...
                                if (dateDiff(val.Date, new Date()) < deltime) {
                                    if (val.Type==4) {
                                        ...
                                        if (val.BackSuccess=='1' && val.Status==3) {                                            
                                                _html += '<a class="text-navy">已拒绝，退款申请已提交(退款时间视银行处理时间而定)</a> ';
                                        }else{                                        
                                            if (val.Status==3) {
                                                _html += '<a class="text-navy" data-cid=' + val.InWarehouseId + ' data-type="pass">已同意入库申请</a> &nbsp;&nbsp;&nbsp;&nbsp; <a class="text-danger par-in-a" data-cid=' + val.InWarehouseId + ' data-type="delete">删除</a>';
                                            }else if (val.Status==4) {
                                                _html += '<a class="text-navy" data-cid=' + val.InWarehouseId + ' data-type="pass">已拒绝入库申请</a> &nbsp;&nbsp;&nbsp;&nbsp; <a class="text-danger par-in-a" data-cid=' + val.InWarehouseId + ' data-type="delete">删除</a>';
                                            }else if (val.Status=='0') {
                                                _html += '<a class="text-navy">未确认的单据</a>';
                                            }else{
                                                if (val.IsPay=='1') {
                                                    _html += '<a class="text-navy">已支付</a>&emsp;<a class="text-navy par-out-a" data-paying="yes" data-cid=' + val.InWarehouseId + ' data-type="pass">同意申请</a> &nbsp;&nbsp;&nbsp;&nbsp; <a class="text-danger par-in-a reforder" data-cid=' + val.InWarehouseId + ' data-type="delete">拒绝申请</a>';
                                                }else{
                                                    _html += '<a class="text-danger">未支付</a>&emsp;<a class="text-navy par-out-a" data-cid=' + val.InWarehouseId + ' data-type="pass" data-paying="no">同意申请</a> &nbsp;&nbsp;&nbsp;&nbsp; <a class="text-danger par-in-a reforder" data-cid=' + val.InWarehouseId + ' data-type="delete">拒绝申请</a>';
                                                }
                                            }
                                        }
                                    }else if (val.Type==5) {
                                        _html += '<a class="text-navy" data-cid=' + val.InWarehouseId + ' >等待商户确认入库</a> ';
                                    }else{
...
$(document).on('click','.reforder',function(){
    var _this=$(this);
    var cid=_this.attr('data-cid');
    art.dialog.confirm('確定要拒絕入庫申請嗎？已付款單據將自動退款，是否繼續？',function(){
        $.ajax({
            url:reforder,
            type:"post",
            data:"cid="+cid,
            ...
            success:function(msg){
                if (msg.status=='success') {
                    _this.parent().html('<a class="text-danger">已拒絕申請</a>');
                }else{
