$(document).ready(function() {
    ...
    $(document).on("click", "#chkAll", function() {
        ...
        outwarehouselist.find('input[class="cans"]').each(function() {
    ...
    $("#btnOkOut").bind("click", function() {
        ...
        if (typest=='1') {
            $.ajax({
                url:bindDefaultInfo.postUpdateUrl,
                type:"post",
                data:$('#out_warehouse_form').serialize(),
                dataType:"json",
                success:function(msg){
                    if (msg.status=='success') {
                        $('#btnOkOut').html('提交出库单据').attr('data-st','0');
                    }else{
                        art.dialog.tips('处理失败，请检查数据');
                    }
                }
            })
        }else{
            ...
            var _len = $("#outwarehouselist tr").find('input[class="cans"]').length;
            if (_len == 0) {
                art.dialog.alert('未找到已选出库商品规格信息！'); // 提示语句
                return false;
            }
            ...
            if ($('#ispass').val()=='1') {
                var attrss = [];
                outwarehouselist.find('input[type="checkbox"]:checked').each(function() {
                    passdata+=$(this).prop('value')+",";
                    attrss.push({
                        "card": $.trim($(this).prop('value'))
                    });
                });
                if (attrss.length == 0) {
                    art.dialog.alert('请选择出库商品！'); // 提示语句
                    // 提示语句
                    return false;
                }
            };
...
//出库单修改
$(document).on('keyup','.out_num',function(){
    var _this=$(this);
    var pid=_this.attr('data-pid');
    var tmpprice=$('.out_price'+pid).val();
    var num=_this.val();
    var money=parseFloat(tmpprice)*parseFloat(num);
    $('.out_money'+pid).val(money);
    //改变按钮状态
    $('#btnOkOut').html('保存出库单更改数据').attr('data-st','1');
})
$(document).on('keyup','.out_price',function(){
    var _this=$(this);
    var pid=_this.attr('data-pid');
    var tmpprice=_this.val();
    var num=$('.out_num'+pid).val();
    var money=parseFloat(tmpprice)*parseFloat(num);
    $('.out_money'+pid).val(money);
    $('#btnOkOut').html('保存出库单更改数据').attr('data-st','1');
})
