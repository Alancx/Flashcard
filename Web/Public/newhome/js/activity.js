$(document).ready(function(){
  $('.proinfo>img').css('height',$('.proinfo>img').width());
  // 点击已点
...
function setcartslist(){
  if (cartslist!='') {
    ...
    $.each(cartsArray,function(index,item){
      ...
      '</div>';
      if (item['NumType']=='1') {
        $('.proinfo[data-pid='+item['ProId']+'][data-plid='+item['ProIdCard']+']').find('.minusbtn').css('display','block');
        $('.proinfo[data-pid='+item['ProId']+'][data-plid='+item['ProIdCard']+']').find('.showselnum').css('display','block');
        $('.proinfo[data-pid='+item['ProId']+'][data-plid='+item['ProIdCard']+']').find('.showselnum').text(item['snums']);
      } else {
        $('.proinfo[data-pid='+item['ProId']+'][data-plid='+item['ProIdCard']+']').find('.proweight').val(parseFloat(item['snums']));
      }
    });
...
function setselprosinfos(pid,snum,plid,pspec,cid,numtype){
  ...
  if ($('.selproslist>.inproinfo[data-pid='+pid+'][data-plid='+plid+']').length<=0) {
    var objdiv = $('.proinfo[data-pid='+pid+']>.shownumbtn');
    if (numtype=='1') {
...
function setselspec(){
  ...
  $('.selSpec').on('tap',function(){
    ...
    var numtype = $(this).parent('.shownumbtn').attr('data-numtype');
    var plist = JSON.parse(allproinfo)[pid]['prolist'];
    if (plist.length > 0) {
