...
$(document).ready(function(){
  // 首次进来定义商品区域的高度信息
  $('.allinfopart>div').css('height',$('.allproinfo').height() - $('.tabpart').height());
  // 页头背景
  var itimg = $(".shoplogo").attr("src");
  $(".bgimg").attr("src", itimg);
  // 红包区域宽度设置
  var rednums = $('.redenvel_list>.redinfo').length;
  if (rednums > 0) {
    $('.redenvel_list').css('width',rednums * 70 + 'px');
  } else {
    $('.redenvellist').css('display','none');
  }
  // 左侧分类默认第一个被选中
  $('.lefttab>ul>li:first-child').addClass('classactive');
  //点菜-评论 -商家
  mui('#tab_part').on('tap', 'li', function() {
    if (!$(this).hasClass('current')) {
      $(this).addClass("current").siblings().removeClass("current");
      var i = $(this).index();
      $(".allinfopart>div[data-oo='tab']").fadeOut(0).eq(i).fadeIn(0);
      if ($('.allproinfo').css('overflow') == 'auto') {
        $('.propart>div').css('overflow','hidden');
        $('.evaluatepart').css('overflow','hidden');
        $('.businesspart').css('overflow','hidden');
      } else {
        $('.propart>div').css('overflow','hidden');
        $('.evaluatepart').css('overflow','hidden');
        $('.businesspart').css('overflow','hidden');
  ...
  // 路线规划
  $('.setshoplx').on('tap',function(){
    var sname=$(this).attr("data-sname");
    var lat=$(this).attr("data-lat");
    var lng=$(this).attr("data-lng");
    var hrefpath="http://apis.map.qq.com/tools/routeplan/eword="+sname+"&epointx="+lng+"&epointy="+lat+
    "&footdetail=2&navibutton=1&editstartbutton=0"+
...
// 设置左侧滚动事件
function lefttabscroll(){
  $(".lefttab").off("scroll");
  $(".lefttab").on("scroll", function() {
    var _this = $(this);
    // 滚动到顶部触发
    if(_this[0].scrollTop<=0){
      if ($('.current').index() == '0') {
        $('.propart>div').css('overflow','hidden');
      };
      $('.contentstop').css('display','block');
      $('.allproinfo').scrollTop($('.contentstop').height()-1);
      $('.allproinfo').css('overflow','auto');
    }
  });
}
// 设置左侧滚动事件
function righttabscroll(){
  $(".righttab").off("scroll");
...
function evaluatescroll(){
  $('.evaluatepart').off('scroll');
  $('.evaluatepart').on('scroll',function(){
    var _this = $(this);
    // 滚动到顶部触发
    if(_this[0].scrollTop<=0){
      if ($('.current').index() == '1') {
...
function businessscroll(){
  $('.businesspart').off('scroll');
  $('.businesspart').on('scroll',function(){
    var _this = $(this);
    // 滚动到顶部触发
    if(_this[0].scrollTop<=0){
      if ($('.current').index() == '2') {
  ...
  if (selproslist.length>0) {
    ...
    selproslist.each(function(index,item){
      if ($(item).find('.shownumbtn').attr('data-numtype')=='1') {
        ...
        // spronum +=parseFloat($(item).find('.shownumbtn>.proweight').val());
        spronum +=1;
...
function setshopcollect(ctype){
  ...
  $.ajax({
    ...
    success:function(msg){
      if (msg['status'] == 'true') {
        if (msg.info=='1') {
          $('.setshopgz').attr('data-type','1');
          $('.setshopgz').children('small').attr('class','ggg xin_fillicon');
          $('.setshopgz').children('span').text('已关注');
        } else {
          $('.setshopgz').attr('data-type','0');
          $('.setshopgz').children('small').attr('class','ggg xin_icon');
          $('.setshopgz').children('span').text('关注');
        }
      }
    },
    error:function(e){
