...
TBL;
	...
	public function ScanPay(){
		...
		$stores=M()->table('RS_Store')->where("token='%s' and IsCheck='%s'",array($this->token,'1'))->select();
	...
	public function showQr(){
		...
		$mid=$_GET['mid'];
		if ($type=='scanpay') {
			$url='http://'.$_SERVER['SERVER_NAME'].'/Smodel/Base/?token='.$this->token.'&sid='.$sid.'&mid='.$mid;
		}
		if ($type=='msg') {
	...
	public function saveScanPro(){
		...
		if (M()->table('RS_Product')->where("token='%s' and ProId='%s'",array($this->token,$pid))->setField('IsScan',$info)) {
			if ($type=='add') {
				$this->success('设置成功');
			}else{
