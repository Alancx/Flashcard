        ...
        <div class="ibox float-e-margins">
          <div class="col-lg-12 col-md-12" style="border-bottom:0px solid #ccc;padding-bottom:10px;margin-bottom:20px;">
            <form action="" class="form-inline" method="post" id="saveprice">
              ...
              <button class="btn btn-primary btn-outline btn-md">添加</button>
            </form>
          ...
          <tbody>
           <?php if(is_array($prolist)): foreach($prolist as $key=>$pro): ?><tr>
              <td><img src="<?php echo ($pro["ProLogoImg"]); ?>" style="width:100px;height:100px;" alt=""><br><?php echo ($pro["ProName"]); ?></td>
              <td><button class="btn btn-danger btn-xs delete" data-id='<?php echo ($pro["ID"]); ?>'>删除</button></td>
...
$(document).ready(function(){
  ...
  $(document).on('click','.delete',function(){
    ...
    art.dialog.confirm('确定要删除此特色商品吗？',function(){
        art.dialog.tips('处理中...');
        $.ajax({
          url:"<?php echo U('Products/delspro');?>",
          type:"post",
          data:"ID="+ID,
          dataType:"json",
