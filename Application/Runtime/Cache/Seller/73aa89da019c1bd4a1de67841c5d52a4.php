				...
				<div class="col-sm-12">
					<div class="ibox float-e-margins">
						...
						<form role="form" class="form-inline" id="savebuy"  method="post" action="">
							...
							<div class="form-group">
								<label for="exampleInputPassword2" class="sr-only">选择商品</label>
								<input type="text" name="Price" id="buyprice" class="form-control" placeholder='请填写团购价格'>
							</div>
							<div class="form-group">
								<label for="exampleInputPassword2" class="sr-only">选择商品</label>
								<input type="number" name="buynum" id="buynum" class="form-control" placeholder='团购人数'>
							</div>
				...
				<div class="col-sm-12 col-md-12">
					<table class="table bordered">
						...
						<?php if(is_array($limits)): foreach($limits as $key=>$li): ?><tr>
								...
								<td><button class="btn btn-danger btn-xs clelimit" data-pid='<?php echo ($li["ProId"]); ?>'>取消团购</button></td>
...
$(document).ready(function(){
	...
	$('#savebuy').submit(function(){
		...
		var pid=$('#chosen').val();
		if (strtime && endtime && buynum && pid && price) {
			if (strtime>endtime) {
				art.dialog.tips('无效的时间区间');
				return false;
			}else if (price<=0) {
				art.dialog.tips('请填写正确团购价');
				return false;
			}else if (buynum<=0) {
				art.dialog.tips('团购人数需大于1');
				return false;
			}else{
	...
	$(document).on('click','.clelimit',function(){
		...
		art.dialog.confirm('确定要取消此商品限购设置吗？',function(){
			$.ajax({
				url:"<?php echo U('Products/delgroupon');?>",
				type:"post",
				data:"pid="+pid,
				dataType:"json",
