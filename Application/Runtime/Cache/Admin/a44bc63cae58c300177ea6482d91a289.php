							...
							<div class="controls">
								<select name="merchant" id="merchant" class="form-control input-sm" style="font-size:12px;display:inline-block;">
									...
									<?php foreach ($merchants as $mer): ?>
										<option value="<?php echo ($mer["stoken"]); ?>"><?php echo ($mer["storename"]); ?></option>
									<?php endforeach ?>
								</select>
							</div>
						</div>
										...
										<?php if(is_array($dataOrder)): foreach($dataOrder as $k=>$vo): ?><tr>
											...
											<tr>
												<td colspan="7">
													...
													</span>&emsp; <span>商户：<?php echo ($vo["storename"]); ?></span>
...
<script>
	var binddefaultinfo={
		...
		postUrl:"<?php echo U('merorder');?>",
		postExport:"<?php echo U('exportorderstoken');?>",
...
<script src="/Public/Admin/Admin/common/js/merorder.js?v2.15"></script>
