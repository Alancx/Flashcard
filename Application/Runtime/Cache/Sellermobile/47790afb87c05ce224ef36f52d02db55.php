...
<link rel="stylesheet" href="/Public/Sellermobile/css/orders.css?v=1.9">
<style>
.nodata{
  display: block;
...
<?php if(empty($orderlist)): ?><div class="nodata">
  ...
  <?php else: ?>
  <?php if(is_array($orderlist)): foreach($orderlist as $key=>$oitem): ?><div class="orderinfo">
      <label class="orderid"><?php echo ($oitem['MemberName']); ?>
      <?php switch($oitem["Status"]): case "2": ?><span>待发货</span><br><?php break;?>
            <?php case "3": ?><span>待收货</span><br><?php break;?>
            <?php case "5": ?><span>退款中</span><br><?php break;?>
            <?php default: ?><span>未完成</span><br><?php endswitch;?>
      ...
      <label class="totalprice">共<?php echo ($oitem['Count']); ?>件,合计:<span>￥<?php echo $oitem['Price']?sprintf("%.2f",$oitem['Price']):'0.00' ?></span>(含运费:<?php echo $oitem['Freight']?sprintf("%.2f",$oitem['Freight']):'0.00' ?>)</label>
