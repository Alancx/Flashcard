...
<?php if(empty($tkorders)): ?><div class="nodata">
    <img src="/Public/Sellermobile/icon/watermark.png" alt="">
  </div>
  <?php else: ?>
  <div class="ordercontents">
    <?php if(is_array($tkorders)): foreach($tkorders as $key=>$info): ?><div class="orderinfo order_<?php echo ($info['OrderId']); ?>">
        <label class="orderid"><?php echo ($info['MemberName']); ?><span>待退款</span><br><span><?php echo ($info['OrderId']); ?></span><span><?php echo ($info['oDate']); ?></span><br>
        ...
        <div class="btn_group">
          ...
          <label class="tkjjorder" data-oid="<?php echo ($info['OrderId']); ?>">拒接退款</label>
        </div>
      </div><?php endforeach; endif; ?>
  </div><?php endif; ?>
<div class="vercouver">
  <div>
    <label>填写验证码</label>
    <div class="inputver">
      <input class="vercode" type="number" name="" value="" placeholder="填写验证码">
      <img class="verimg" src="" alt="">
    </div>
    <div class="btn_ver">
      <label class="qxver">取消</label>
      <label class="suerver" data-oid="">确定</label>
    </div>
  </div>
</div>
<script type="text/javascript">
  var verify_url="<?php echo U('Orders/getverify');?>";
  var tk_url="<?php echo U('Orders/settkorder');?>";
  var index_url="<?php echo U('Index/Index');?>";
</script>
<script src="/Public/Sellermobile/js/tkorders.js?v=1.3" charset="utf-8"></script>
