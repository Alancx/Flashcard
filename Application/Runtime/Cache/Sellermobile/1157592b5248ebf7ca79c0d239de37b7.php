...
<link rel="stylesheet" href="/Public/Sellermobile/css/Factoryproadd.css?v=1.0">
<div class="Factoryproaddcontent">
  <div class="prodata">
    <img src="<?php echo ($proinfo[ProLogoImg]); ?>" alt="">
    <label class="proname"><?php echo ($proinfo['ProName']); echo ($proinfo['ProTitle']); ?></label>
  </div>
  <div class="proattr">
    <label class="attrprice">价格设置</label>
    <?php if(is_array($proinfo['prolist'])): foreach($proinfo['prolist'] as $key=>$proattr): ?><div class="pro_attr">
        <div class="attr_item">
          <label>商品规格:</label>
          <input type="text" name="" value="<?php echo ($proattr['ProSpec1']); ?>" readonly="true">
        </div>
        <div class="attr_item">
          <label>工厂售价:</label>
          <input type="text" name="" value="<?php echo ($proattr['Price']); ?>" readonly="true">
        </div>
        <div class="attr_item setattr_price" data-proidcode="<?php echo ($proattr['ProIdCard']); ?>">
          <label>设置售价:</label>
          <input type="number" name="" value="<?php echo ($proattr['setprice']); ?>" class="setprice" placeholder="设置销售价格">
        </div>
      </div><?php endforeach; endif; ?>
  </div>
  <div style="height:50px;"></div>
  <div class="btn_grup">
    <label class="saveselpro" data-proid="<?php echo ($proinfo['ProId']); ?>">保存到我的小店</label>
  </div>
</div>
<script type="text/javascript">
  var savefactorypro_url="<?php echo U('Products/savefactorypro');?>";
  var factorypro_url="<?php echo U('Products/prolist');?>";
  var factype="<?php echo ($factype); ?>";
</script>
<script src="/Public/Sellermobile/JS/Factoryproadd.js?v=1.0" charset="utf-8"></script>
