...
//http://wx.58ate.com/Home/Account/QrCodeHandle/tp/3/gid/pro0515775329_38/sid/11.html
    ...
    $gInfo=M()->table(C('DB_BASE')['DB_PREFIX']."product p")->join(C('DB_BASE')['DB_PREFIX']."productlist pl ON p.ProId=pl.ProId","LEFT")->join(C('DB_BASE')['DB_PREFIX']."productonsale pos ON pos.ProId=pl.ProIdCard AND pos.Ison=1 AND (GETDATE() BETWEEN pos.stime AND pos.etime)","LEFT")->where($gInfoCondition)->field("pl.ProIdCard AS pid,p.ProName AS pname,pl.Price AS price,pos.sprice AS sprice,p.IsUseScore,p.Score,p.stoken")->find();
    if ($gInfo)
    {
...
//  );
      ...
      if ($gInfo['stoken']!=$this->stoken) {
        $nowOrderArray['stoken']=$gInfo['stoken'];
      }
  ...
  public function storeCashier()
  {
      ...
      // var_dump($_COOKIE);
            ...
            $orderData['stoken']=$this->stoken;
            $orderData['Prostoken']=$orderInfoList['stoken'];
